...
for (let i = 0; i < 10; i++) {
  describeChromeOnly('TargetManager', () => {
    ...
    it('should handle targets', async () => {
      ...
      expect(targetManager.attachedTargets().size).toBe(2);
      ...
      expect(targetManager.attachedTargets().size).toBe(1);
      ...
      await framePromise;
      expect(await browser.pages()).toHaveLength(1);
      expect(targetManager.attachedTargets().size).toBe(3);
      ...
      await framePromise;
      expect(await browser.pages()).toHaveLength(1);
      expect(targetManager.attachedTargets().size).toBe(4);
